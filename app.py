...
def home():
    try:
        return jsonify("Welcome to Med Inv API")
    except Exception as e:
        app.logger.error(e)
...
def sales():
    try:
        ...
        amountInHand_online = sum(
            float(sale['amount_accepted']) for sale in sales if sale.get('payment_mode') == 'online')
